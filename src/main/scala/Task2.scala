...
object Task2 extends App {
  ...
  def processtaxiRDD (taxiRDD: RDD[Row]) = {
  ...
  val taskresult = processtaxiRDD(TaxiFactRDD)

  val value1 = taskresult.foreach(x => println(x))

  val pathout = "src/main/resources/data/result_data/task2result"

  val resultdirectory = new Directory(new File(pathout))

  if (resultdirectory.exists) { resultdirectory.deleteRecursively() }

  val value2 = taskresult.saveAsTextFile(pathout)
